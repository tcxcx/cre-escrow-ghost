...
export function SigningView({ contractId }: SigningViewProps) {
  ...
  const commissionTotal = contract.commissions.reduce((sum, c) => sum + c.percentage, 0)
          ...
          <CardContent>
            <div className="grid gap-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {currentUserRole === 'payer' ? contract.payee.bufiHandle : contract.payer.bufiHandle}
                </span>
              </div>
              <div className="flex justify-between text-sm">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <SignatureStatusList
              ...
              payerSignature={contract.payerSignature}
              payeeSignature={contract.payeeSignature}
            ...
            <Card className="mt-6">
              <CardContent className="pt-6">
                ...
                <Button
                  ...
                  disabled={!termsAgreed || signingState !== 'idle'}
                  onClick={handleSign}
                >
                ...
                <p className="text-center text-xs text-muted-foreground mt-3">
                  Signs with your connected wallet on {contract.chain}
                  <br />
                  Estimated gas: ~0.002 AVAX
                </p>
              </CardContent>
            </Card>
          </>
            ...
            <Card className="mb-6">
              ...
              <CardContent>
                <div className="space-y-2">
                  {contract.payerSignature && (
                    <div className="flex items-center justify-between p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                      <div className="flex items-center gap-2">
                        <Check className="w-4 h-4 text-emerald-500" />
                        <span className="text-sm font-medium">{contract.payer.name}</span>
                      </div>
                      <span className="text-xs text-muted-foreground">
                        {contract.payerSignature.signedAt && format(contract.payerSignature.signedAt, 'MMM d, h:mm a')}
                      </span>
                    </div>
                  )}
                  {contract.payeeSignature && (
                    <div className="flex items-center justify-between p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                      <div className="flex items-center gap-2">
                        <Check className="w-4 h-4 text-emerald-500" />
                        <span className="text-sm font-medium">{contract.payee.name}</span>
                      </div>
                      <span className="text-xs text-muted-foreground">
                        {contract.payeeSignature.signedAt && format(contract.payeeSignature.signedAt, 'MMM d, h:mm a')}
                      </span>
                    </div>
                  )}
                </div>
