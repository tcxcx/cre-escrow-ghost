...
export function ContractPreview() {
  ...
  const shareLink = useMemo(() => {
    if (!currentSavedContract) return null
    return generateShareLink()
  }, [currentSavedContract, generateShareLink])
  ...
  const { setCurrentSavedContract } = useContractStore()
  ...
  const deployAgreement = async () => {
    ...
    try {
      ...
      const agreementId = response.agreementId ?? (response as Record<string, unknown>).agreement_id as string | undefined
      if (agreementId) {
        const shareData = (response as Record<string, unknown>).shareData as {
          token?: string
          shortLink?: string
        } | null
        setCurrentSavedContract({
          id: agreementId,
          title: contractName,
          shortLink: shareData?.shortLink,
            ...
            <div className="grid grid-cols-2 gap-4 p-4 rounded-lg bg-secondary/50 mb-6">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground capitalize">
                  {settings.yieldStrategy.enabled ? settings.yieldStrategy.strategyName : 'No Yield'}
                </p>
              </div>
              <div>
