...
import { listAgreements, type AgreementSummary } from '@/lib/api/client'
...
const COMPLETED_MILESTONE_STATES = new Set(['APPROVED', 'RELEASED', 'PAID'])

function extractPartyName(row: AgreementSummary, role: 'payer' | 'payee'): string {
  const fallback = role === 'payer' ? 'Payer' : 'Payee'
  const json = row.agreement_json
  if (!json || typeof json !== 'object') return fallback
  const parties = (json as { parties?: unknown[] }).parties
  if (!Array.isArray(parties)) return fallback
  const party = parties.find(
    (p): p is { role: string; name: string } =>
      typeof p === 'object' && p !== null && (p as { role?: string }).role === role
  )
  return party?.name || fallback
}

function extractMilestoneCounts(row: AgreementSummary): { completed: number; total: number } {
  const milestones = row.milestones
  if (!Array.isArray(milestones) || milestones.length === 0) return { completed: 0, total: 0 }
  const completed = milestones.filter((m) => COMPLETED_MILESTONE_STATES.has(m.state)).length
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setContracts(
          rows.map((row) => {
            const { completed, total } = extractMilestoneCounts(row)
            return {
              ...
              currency: 'USDC',
              payerName: extractPartyName(row, 'payer'),
              payeeName: extractPartyName(row, 'payee'),
