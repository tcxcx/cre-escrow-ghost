...
import { VerificationProgress } from '@/components/contract/contracts/submission/verification-progress'
...
export function MilestoneDetail({ contractId, milestoneId }: MilestoneDetailProps) {
  ...
  useEffect(() => {
    ...
    fetchContract(contractId).catch((err) => {
      setFetchError(err instanceof Error ? err.message : 'Failed to load milestone')
    })
  ...
  const canSubmit = () => {
    ...
    return hasFiles && checklistComplete && (submissionState === 'idle' || submissionState === 'error')
  }
...
useEffect(() => {
    if (submissionState === 'success' && contract && milestone) {
      toast({ title: 'Deliverable submitted successfully', variant: 'success-light' })
      ...
      router.push(`/contracts/${contract.id}/milestones/${milestone.id}/verification`)
    }
    if (submissionState === 'error') {
      toast({ title: 'Submission failed. Please try again.', variant: 'error-light' })
    }
  }, [submissionState, resetSubmissionState, contract, milestone, router])
  ...
  if (loading || !contract || !milestone) {
    return (
      <div className="min-h-screen flex items-center justify-center gap-2">
        ...
        <span className="text-sm text-muted-foreground">Loading milestone...</span>
      </div>
    )
  }
      ...
      <Sheet open={showSubmitPanel} onOpenChange={setShowSubmitPanel}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          ...
          <div className="mt-6 space-y-6">
            ...
            {/* Verification Progress (shown during submission) */}
            ...
            <div className="space-y-3">
              ...
              >
                ...
                {submissionState === 'error' && 'Retry Submission'}
